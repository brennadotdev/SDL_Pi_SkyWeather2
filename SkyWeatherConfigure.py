...
class SkyWeatherConfigure(App):
    ...
    def buildScreen1(self):
        ...
        self.F_SWDEBUG = gui.CheckBoxLabel('enable SW Debugging', self.SWDEBUG, height=30,
                                           style='margin:5px; background: LightGray ')
        vbox.append(self.F_SWDEBUG, 'self.F_SWDEBUG')

        # mysql configurattion 
        mysqlheader = gui.Label("MySQL Configuration",
                                style='position:absolute; left:5px; top:40px;' + self.headerstyle)
        vbox.append(mysqlheader, 'mysqlheader')
        self.F_enable_MySQL_Logging = gui.CheckBoxLabel('enable MySQL Logging ', self.enable_MySQL_Logging, height=30,
                                                        style='margin:5px; background:LightGray')
        ...
        self.F_enable_WLAN_Detection = gui.CheckBoxLabel('enable WLAN Detection', self.enable_WLAN_Detection, height=30,
                                                         style='margin:5px; background:LightGray')
        ...
        P6NTheader = gui.Label("English or Metric Units",
                               style='position:absolute; left:5px; top:30px;' + self.headerstyle)
        vbox.append(P6NTheader, 'P6NTheader')

        self.F_English_Metric = gui.CheckBoxLabel('English (not checked) or Metric (checked)', self.English_Metric,
                                                  height=30, style='margin:5px; background: LightGray ')
    ...
    def buildScreen2(self):
        ...
        self.F_enableText = gui.CheckBoxLabel('enable Text Messaging', self.SWDEBUG, height=30,
                                              style='margin:5px; background: LightGray ')
    ...
    def buildScreen3(self):
        ...
        self.F_runLEDs = gui.CheckBoxLabel('Enable Pixel/NeoPixel', self.runLEDs, height=30,
                                           style='margin:5px; background: LightGray ')
        vbox.append(self.F_runLEDs, 'self.F_runLEDs')

        P1Nheader = gui.Label("Solar Max Configuration",
                              style='position:absolute; left:5px; top:30px;' + self.headerstyle)
        vbox.append(P1Nheader, 'P1Nheader')

        self.F_SolarMAX_Present = gui.CheckBoxLabel('SolarMAX Present', self.SolarMAX_Present, height=30,
                                                    style='margin:5px; background: LightGray ')
        vbox.append(self.F_SolarMAX_Present, 'self.F_SolarMAX_Present')

        P2Nheader = gui.Label("Station Height in Meters",
                              style='position:absolute; left:5px; top:30px;' + self.headerstyle)
    ...
    def buildScreen4(self):
        ...
        self.F_USEWEATHERSTEM = gui.CheckBoxLabel('Enable WeatherSTEM', self.USEWEATHERSTEM, height=30,
                                                  style='margin:5px; background: LightGray ')
        vbox.append(self.F_USEWEATHERSTEM, 'self.F_USEWEATHERSTEM')

        p5label = gui.Label("Interval between pictures (seconds)",
                            style='position:absolute; left:5px; top:40px;' + self.labelstyle)
        vbox.append(p5label, 'p5label')
        ...
        P4Nheader = gui.Label("WeatherUnderGround Configuration",
                              style='position:absolute; left:5px; top:30px;' + self.headerstyle)
        vbox.append(P4Nheader, 'P4Nheader')

        self.F_WeatherUnderground_Present = gui.CheckBoxLabel('Enable WeatherUnderground',
                                                              self.WeatherUnderground_Present, height=30,
                                                              style='margin:5px; background: LightGray ')
    ...
    def buildScreen5(self):
        ...
        self.F_USEBLYNK = gui.CheckBoxLabel('Enable Blynk', self.USEBLYNK, height=30,
                                            style='margin:5px; background: LightGray ')
        ...
        vbox.append(P1Nheader, 'P1Nheader')
        self.F_USEWSLIGHTNING = gui.CheckBoxLabel('Use WS Lightning ', self.USEWSLIGHTNING, height=30,
                                                  style='margin:5px; background: LightGray ')
        vbox.append(self.F_USEWSLIGHTNING, 'self.F_USEWSLIGHTNING')
        self.F_USEWSAQI = gui.CheckBoxLabel('Use WS Air Quality ', self.USEWSAQI, height=30,
                                            style='margin:5px; background: LightGray ')
        vbox.append(self.F_USEWSAQI, 'self.F_USEWSAQI')
        self.F_USEWSSKYCAM = gui.CheckBoxLabel('Use WS Sky Camera ', self.USEWSSKYCAM, height=30,
                                               style='margin:5px; background: LightGray ')
        vbox.append(self.F_USEWSSKYCAM, 'self.F_USEWSSKYCAM')
        self.F_ALTWSSKYCAM = gui.CheckBoxLabel('Alternate SkyCam/WS SkyCamera ', self.ALTWSSKYCAM, height=30,
                                               style='margin:5px; background: LightGray ')
    ...
    def buildScreen7(self):
        ...
        self.F_REST_Enable = gui.CheckBoxLabel('REST Enable', self.REST_Enable, height=30,
                                               style='margin:5px; background: LightGray ')
        ...
        self.F_MQTT_Enable = gui.CheckBoxLabel('MQTT Enable', self.MQTT_Enable, height=30,
                                               style='margin:5px; background: LightGray ')
    ...
    def buildScreen8(self):
        ...
        vbox.append(P5Nheader, 'P5Nheader')
        self.F_Camera_Night_Enable = gui.CheckBoxLabel('Night Vision Enable', self.Camera_Night_Enable, height=30,
                                                       style='margin:5px; background: LightGray ')
    ...
    def main(self):
        ...
        cancel = gui.Button('Cancel',
                            style='position:absolute; left:550px; height: 30px; width:100px; margin:10px; top:5px')
        cancel.onclick.do(self.onCancel)
        save = gui.Button('Save',
                          style='position:absolute; left:400px; height: 30px; width:100px;  margin: 10px;  top:5px')
        save.onclick.do(self.onSave)
        exit = gui.Button('Save and Exit',
                          style='position:absolute; left:475px; height: 30px; width:100px;  margin: 10px;  top:95px')
        exit.onclick.do(self.onExit)
        reset = gui.Button('Reset to Defaults',
                           style='position:absolute; left:400px;height: 30px;   width:250px; margin: 10px; top:50px')
...
# Configuration
configuration = {'config_enable_file_cache': True, 'config_multiple_instance': True, 'config_port': 8001,
                 'config_address': '0.0.0.0', 'config_start_browser': False, 'config_project_name': 'untitled',
